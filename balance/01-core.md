...
| `echoResonancePower` | 0.005 | «Отзвук глубины» — бесконечный узел за выкупленным древом (раунд 40). Уровень множит всю силу наёмника на 1+power, как ранг; каждый следующий в costGrowth раз дороже, первый стоит как следующий узел древа. ПЕРВЫЕ ЧИСЛА 0.01 и 1.25 ДАЛИ +38 % РЕКОРДА к 150-му контракту (355 → 496): постоянная прибавка силы раскручивается циклом добычи — снаряжение с глубины D уводит на 1.021·D, — а не остаётся теми парой этажей, что даёт сама. Замер --campaign 150, сиды 42 и 7: 0.01/2 — +6 %, 0.005/1.5 — +9 %, 0.01/3 — +2 %, 0.005/2 — +2 % при 24–25 уровнях. Выбрано 0.005/2: покупка остаётся событием, рекорд почти не сдвигается. |
| `echoResonanceCostGrowth` | 2 |  |
...
| `bossMight` | 8 | Здоровье босса бездны — поправка стража (кривая вещей от глубины аудита) и мощь сверху, набираемая по квадрату глубины до bossMightDepth (раунд 41). Без поправки к шестидесятому контракту босс падал с одного удара, и урон на позднем этапе не стоил ни этажа. Мощь целиком с первого этажа ставила стену на тридцатом этаже первого спуска; поправка в степени росла быстрее урона вещей, и кампания вставала навсегда на 269. |
| `bossMightDepth` | 300 |  |
| `brandMobHpPerRank` | 0 | Прибавка здоровья мобов за ранг Клейма, отдельно от урона (раунд 41). Пока враг гибнул с одного удара, множитель к здоровью ничего не значил; как только здоровье решает — у боссов, — он удлиняет бой и усиливает удар разом и бьёт по рангу вдвое. |
...
| `sellBonus` | 1.6 |  |
| `bossEnrageSeconds` | 3 | Ярость босса бездны (раунд 41): не убит за bossEnrageSeconds — удар растёт на bossEnragePerSecond за каждую следующую секунду. Проверка урона: на волнах решает живучесть, у босса — урон. Ярость ранняя и крутая, а не поздняя: у рубежа сборка убивает босса за две секунды, и ярость с двадцатой не наступала ни в одном бою; с третьей она бьёт ровно ту сборку, которой урона не хватает (замер --late: урон x10 стал давать до +4 этажей, темп кампании прежний). |
| `bossEnragePerSecond` | 0.5 |  |
...
| `relayFirstLevel` | 2 | Смена (раунд 40): уровни Костра, на которых у каждого слота спуска появляется ещё одно место сменщика. Сменщик уходит вниз в секунду гибели предыдущего наёмника и в его сборке — так спуск продолжается, пока игрока нет. |
| `relaySecondLevel` | 5 |  |
| `relayThirdLevel` | 8 |  |
