...
| `passivePointCap` | 60 |  |
| `lairUnlockDepth` | 150 | Логова стражей — лестница позднего этапа (раунд 36). Круг N стоит на глубине lairFirstDepth + lairDepthPerCircle*(N-1); вызов стоит lairOfferingFloors этажей дохода на этой глубине. Первая победа на круге даёт очко пассивок и уникальную вещь босса, повторная — вещь с шансом lairRelicChance. Открываются рекордом lairUnlockDepth. |
| `lairFirstDepth` | 150 |  |
| `lairDepthPerCircle` | 15 |  |
| `lairOfferingFloors` | 60 |  |
| `lairRelicChance` | 0.35 |  |
| `lairReferenceDepth` | 40 | Глубина, на которой стражей выверяет audit_cli --bosses. Глубже здоровье стража растёт по кривой вещей, а не мобов: иначе сборка той же глубины убивала его за десятую долю секунды (замер раунда 36). |
| `lairFirstKillPoints` | 2 | Очки дерева пассивок за первую победу над каждым стражем. Остальные круги очков не дают: четыре стража — не больше восьми очков. |
| `lairGuardianMight` | 3 | Во сколько раз страж в логове крепче и злее выверенного аудитом: здоровье и урон вместе. Аудит мерит форму боя против голого снаряжения, а к логову приходят с деревом, древом и Легендой. |
...
| `wavesPerFloor` | 3 |  |
| `wavesPerBossFloor` | 1 |  |
| `restSecondsBetweenFloors` | 2 | Переход между этажами. Было 5 с при этаже в 5-13 игровых секунд: пауза читалась дольше самого боя. 2 с короче любого этажа и всё ещё дают полоске здоровья подпрыгнуть. Ниже не опускаем: замер 40 ранов даёт 6.1 минуты медианы, а 1.5 с выводит ран из окна 8-20 минут. Глубина не сдвинулась ни на этаж — это была чистая мёртвая пауза, лечение идёт от restHealFraction. |
