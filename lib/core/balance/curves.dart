...
class Curves {
  ...
  /// Во сколько раз дороже стал задаток наёмника на глубине [depth].
  ///
  ...
  ///
  /// **[depth] — не рекорд.** Здесь стояла `maxDepthEver`, и это был
  /// храповик: цена поднималась вместе с рекордом и не опускалась НИКОГДА,
  /// а доход падал вместе с достигнутой глубиной. Кто такое пережил один
  /// раз, уже не выбирался: на рекорде 133 множитель 25×, а ран на глубине
  /// 63 приносит в 83 раза меньше. Считает эту глубину
  /// `PlayerProfile.hireDepth` — медиана последних спусков.
  static double hireCostScale(int depth) => math.max(
        1.0,
        itemScale(depth) / itemScale(hireScaleFromDepth),
      );
