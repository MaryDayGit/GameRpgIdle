...
class SaveCodec {
  ...
  static Map<String, dynamic> encodeProfile(PlayerProfile p) => {
        ...
        // Логова стражей: взятые круги и счётчик вызовов. Счётчик входит в
        // сид боя — без записи проигранный бой переигрывался бы перезапуском.
        'lairCircles': p.lairCircles,
        'lairAttempts': p.lairAttempts,
  ...
  static PlayerProfile decodeProfile(
    ...
    final profile = PlayerProfile(
      ...
      // Страж, исчезнувший из контента, свой круг не теряет: запись лежит,
      // и вернись он патчем — вернётся и взятое. Очки пассивок за такой круг
      // тоже остаются: отнять их значило бы отнять узлы дерева.
      lairCircles: {
        for (final e in _map(j['lairCircles']).entries)
          if (e.value is num && (e.value as num) > 0)
            e.key: (e.value as num).round(),
      },
      lairAttempts: _int(j['lairAttempts']),
