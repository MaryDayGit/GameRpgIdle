...
class Roster {
  ...
  /// Смена: наняты и ждут у Костра, чтобы уйти вниз вместо павшего (GDD §9.4).
  ///
  /// Список — очередь: первым уходит тот, кто встал в неё первым. Отдельно от
  /// резерва, потому что сменщика нельзя ни одеть, ни отправить руками: он
  /// уходит в чужой сборке и в чужую секунду.
  final List<Mercenary> relay = [];
  ...
  /// Сменщик уходит вниз. Лимита слотов здесь нет намеренно: он занимает
  /// место павшего, чья добыча ещё ждёт, — слот тот же, человек другой.
  void relieve(Mercenary m) {
    relay.remove(m);
    deployed.add(m);
  }
